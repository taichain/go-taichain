...
// Package geth contains the simplified mobile APIs to The go-taichain.
//
// The scope of this package is *not* to allow writing a custom Ethereum client
// with pieces plucked from The go-taichain, rather to allow writing native dapps on
...
// from the The go-taichain repository. Please adhere to this scoping to prevent this
// package getting unmaintainable.
//
// Wrapping guidelines:
//
// Every type that is to be exposed should be wrapped into its own plain struct,
// which internally contains a single field: the original The go-taichain version.
